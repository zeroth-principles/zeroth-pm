...
def test_cumulative_return_with_valid_input():
    rr = RandomReturn(params={'seed': 0, 'freq': 'B', 'distribution': partial(np.random.normal, loc=0.0, scale=1.0)})
    operand = rr(entities=['A', 'B'], period=('2022-01-01', '2022-01-30'))

    func = CumulativeReturn_g_index(params={'index': pd.date_range('2022-01-01', '2022-01-30', freq='w-MON')})
...
def test_cumulative_return_without_index():
    func = CumulativeReturn_g_index(params={})
    with pytest.raises(ValueError, match="grouping index should be specified"):
        func()